...
**The Secrets.** The chart never makes a Secret, because a secret in a values
file is a secret in a Git repository. Make them with Sealed Secrets, External
Secrets, SOPS, or by hand, and name them under `secrets`. Every credential
comes through a `secretKeyRef`; the chart holds no other path to one.
